...
function SearchFilter() {
  ...
  const search = location.state?.toString()
  ...
  let KEYS = [ "name" , "features" , "location" , "description" ]
  ...
  const Search = (posts) =>{
    return posts?.filter((item)=>(
        KEYS.some((key) => item[key]?.toLowerCase()?.includes(search?.toString()))
      ))
  }
      ...
      {length === 0 ?
        ...
        Search(posts)?.map((item)=>(
          ...
          >
            ...
            <LazyLoadImage
              effect='blur'
              className='relative w-[30rem] h-[32rem] object-cover z-[87]'
              src={item?.image}
